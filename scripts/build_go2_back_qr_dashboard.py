...
<div class="table-scroll"><table class="live"><thead><tr><th>시간</th><th>종단 QR 오차</th><th>스키드</th><th>최대 관입</th><th>Go2 최대 기울기</th><th>지형 높이</th></tr></thead><tbody><tr><td>{fmt(summary.get('duration_s'), 1, ' s')}</td><td>{fmt(summary.get('terminal_qr_error_m'), 4, ' m')}</td><td>{fmt(summary.get('max_skid_contacts'), 0, ' / 2')}</td><td>{fmt(float(summary.get('max_penetration_m', float('nan'))) * 1000, 3, ' mm')}</td><td>{fmt(summary.get('max_go2_tilt_deg'), 2, '°')}</td><td>{fmt(summary.get('terminal_terrain_height_m'), 3, ' m')}</td></tr></tbody></table></div>
<p class="offline-note">모든 접촉·관입·지형높이 값은 <code>offline_sim_*</code> 사후 물리 진단입니다. X500 정책 입력에는 Go2 상태나 지형 정답값이 들어가지 않습니다.</p>
...
<div class="split"><div class="formula-card"><h3>지형은 무엇으로 만들었나</h3><p>공개 경사는 길이 16 m의 <strong>10% grade (5.71°) 회전 box collision surface</strong>입니다. 요철은 폭 2.4 m의 <strong>연속 MuJoCo heightfield 충돌면</strong>이며, 수직 타일 벽이 아닌 걸을 수 있는 연속 기복으로 모델링했습니다. 높이 진폭은 <strong>24 / 48 / 80 mm</strong>입니다. 바닥색이나 영상 효과가 아니라 발 접촉·마찰·정상력을 직접 만듭니다.</p><p>상승·하강 모두 12 m 코스를 통과하기 전, 유한 collision surface 끝으로 걸어 내려가지 않도록 감속·정지합니다.</p></div><div class="formula-card"><h3>무엇을 검증했나</h3><p>Go2 저수준 PPO 후보의 상태 계약은 IMU·관절·속도명령·직전 행동 450D입니다. 다만 이 지형 공개 세트는 후보가 낙상 기준을 통과하지 못했으므로, 실제 IMU·자체 odometry 되먹임의 물리 기준 트로트를 사용합니다. 지형 높이·QR pose·드론 상태는 Go2 제어 입력에 넣지 않고, Go2 root wrench는 항상 0입니다.</p><p>아래 영상은 결합 Go2+X500 물리 장면에서 고정 시드를 사전 재생해 <strong>Go2 낙상 0, 최고 기울기 40° 이하, 두 스키드 물리 접촉, root wrench 0</strong>을 통과한 재현입니다. 이는 넓은 무작위화 강건성 인증이 아니라 공개된 고정 시드 재생 검증입니다.</p></div></div>
<p>각 영상은 좌측 3인칭과 우측 하향 카메라를 같은 5 ms MuJoCo 상태에서 녹화합니다. 경사 상승·하강과 요철 1/2/3단계별로 PPO·DDPG·SAC를 실행했으므로 아래에 총 15개 MP4가 있습니다.</p>''' + "".join(panels) + "</section>"
...
<section class="section"><h2>초급·중급·고급: 9개 동기화 영상과 평가</h2><p>각 MP4는 5 ms 물리 서브스텝에서 실제 30 fps로 캡처하며, 좌측 Go2·X500 3인칭과 우측 X500 하향 카메라를 같은 MuJoCo state에서 렌더링합니다. 우측 하단뷰는 검정 레터박스 없이 640×720 전체 높이를 채웁니다. 좌측의 얇은 X500 추적 테두리는 매 프레임 현재 GL 카메라로 투영하고, 1 Hz MuJoCo segmentation 실루엣으로 실제 렌더 가시성과 크기를 반복 검증합니다. 프로펠러는 영상에서만 반대 방향으로 회전하며, Go2와 QR 장착부의 물리에는 영향을 주지 않습니다.</p>{cards}</section>
{terrain_html}
