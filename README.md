...
아닌 폭 2.4 m의 연속 heightfield 충돌면입니다.
...
최대 기울기 40° 이하, root wrench 0, 좌/우 스키드 두 개의 물리 접촉, 최대 관입 2.1 mm
...
세 드론 정책은 서로 같은 `float32[7]`을 입력으로 받고 2D 수평 속도 잔차를 출력한다.
...
생성한다. 각 프레임의 왼쪽은 Go2+X500 3인칭, 오른쪽은 X500 하향 카메라이며 같은 5 ms
MuJoCo 상태에서 기록한다. 하단뷰는 검정 레터박스 없이 전체 패널을 채운다.
