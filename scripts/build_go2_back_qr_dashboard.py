...
<div class="table-scroll"><table class="live"><thead><tr><th>명령 속도</th><th>QR 종단 오차</th><th>Go2 이동창 속도</th><th>Go2 경로</th><th>고무 발바닥</th><th>발바닥 최대 힘</th><th>비발바닥 접지</th><th>최대 기울기</th><th>스키드</th></tr></thead><tbody><tr><td>{fmt(record.get('command_speed_mps'), 2, ' m/s')}</td><td>{fmt(summary.get('terminal_qr_error_m'), 4, ' m')}</td><td>{fmt(summary.get('terminal_motion_window_speed_mps'), 3, ' m/s')}</td><td>{fmt(summary.get('terminal_go2_path_distance_m'), 2, ' m')}</td><td>{fmt(summary.get('max_go2_sole_contacts'), 0, ' / 4')}</td><td>{fmt(summary.get('max_go2_sole_normal_force_n'), 1, ' N')}</td><td>{fmt(summary.get('max_go2_nonsole_terrain_contacts'), 0)}</td><td>{fmt(summary.get('max_go2_tilt_deg'), 2, '°')}</td><td>{fmt(summary.get('max_skid_contacts'), 0, ' / 2')}</td></tr></tbody></table></div>
<p class="offline-note">공식 Unitree Go2의 <strong>FR/FL/RR/RL 고무 발 접촉 형상만</strong> 지지로 인정합니다. 종아리·허벅지·힙·몸통 collision shape가 자갈에 한 번이라도 닿으면 성공을 금지합니다. 표의 Go2·접촉 수치는 정책 입력이 아닌 사후 물리 검증값입니다.</p>
...
<div class="callout"><strong>같은 자갈길·같은 드론 시작 조건에서 속도만 바꿨습니다.</strong> 초급/중급/고급 Go2 전진 명령은 각각 <strong>0.58 / 0.75 / 0.92 m/s</strong>입니다. Go2는 공식 모델의 FR/FL/RR/RL <strong>고무 발바닥만</strong> 자갈길 지지로 쓸 수 있습니다. 종아리·허벅지·힙·몸통이 닿거나 Go2가 멈추거나 넘어지면, QR 데크·Go2 보수 외곽 중 하나라도 길 밖으로 나가면, 또는 실제 스키드 두 개가 QR 판에 닿지 않으면 그 재생은 착륙 성공으로 기록되지 않습니다.</div>
<div class="split"><div class="formula-card"><h3>길은 실제 개별 충돌 돌입니다</h3><p>MuJoCo 물리 코스는 <strong>{length} × {width}</strong>입니다. 다져진 흙 기반에는 <strong>정적 타원체 돌 {rocks}개</strong>가 각각 독립 collision geometry로 묻혀 있고, 바닥 자체도 <strong>{grade} 완만 경사</strong>와 <strong>{undulation}</strong> 미세 기복을 가집니다. 돌·흙·QR 판·Go2 발·X500 스키드는 같은 MuJoCo 충돌계에서 계산됩니다.</p><p>이전 연속 요철 재생은 이 공개 구역에 넣지 않았습니다. 아래는 개별 돌 자갈길 결과만 표시합니다.</p>{preview_html}</div><div class="formula-card"><h3>착륙을 막는 사전 선언 gate</h3><p>각 PPO/DDPG/SAC 영상은 난이도별로 한 개의 고정 시드만 사용했습니다. <strong>낙상 0</strong>, <strong>종아리·허벅지·힙·몸통의 지형 접촉 0</strong>, <strong>코스 이탈 0</strong>, <strong>Go2 root 외력 0</strong>, <strong>최대 기울기 ≤ 35°</strong>, <strong>스키드 2/2</strong>, <strong>수치 관입 ≤ 2.1 mm</strong>, 그리고 착륙 직전 1초 이동창 <strong>≥ 0.12 m/s</strong>를 모두 검사합니다. 따라서 Go2가 멈췄거나 발바닥 외 링크로 버틸 때의 착륙은 통과할 수 없습니다.</p><p>{training_note}</p></div></div>
...
def px4_flat_hil_section() -> str:
    ...
    return '''<section class="section" id="px4-flat-hil"><div class="section-kicker">REAL PX4 SITL · EKF2 · MAVLINK HIL · FLAT ONLY</div><div class="algorithm-heading"><div><h2>분리 실행 PX4 EKF2 평지 착륙</h2><p>아래 12개 카드는 PPO·DDPG·SAC·MPC의 초급·중급·고급을 각각 독립 PX4 SITL 프로세스에서 실행한 결과입니다. MuJoCo는 센서·기체 물리·Go2/QR 접촉을 맡고, PX4는 실제 EKF2·멀티콥터 위치 제어·모터 할당을 수행합니다.</p></div><b class="ok">3D 수평·수직 제어 · 실제 PX4 EKF2 HIL</b></div>
<div class="split"><div class="formula-card"><h3>실제로 PX4에 들어간 값</h3><p>매 5 ms MuJoCo IMU(가속도·자이로·자력계), 기압, GPS 위치·속도를 <code>HIL_SENSOR</code>/<code>HIL_GPS</code>로 보냅니다. PX4 EKF2의 local position·velocity·attitude를 다시 받아 카메라 기반 임무의 드론 자체 상태로만 사용합니다. GPS/상태/명령 좌표는 MuJoCo NWU와 PX4 NED/FRD 사이에서 변환합니다.</p><p>PPO·DDPG·SAC ONNX는 QR/PnP 6개와 PX4 수직속도 1개의 <code>float32[7]</code>만 받고, <strong>3D 제한 residual</strong> <code>[Δv<sub>x</sub>, Δv<sub>y</sub>, Δv<sub>z</sub>]</code>을 냅니다. <strong>MPC는 ONNX 없이</strong> 카메라/PnP 상대 위치·상대 속도와 PX4 EKF 3축 속도로 8-step 3D 속도 계획을 매 100 ms 다시 풉니다. 네 방법 모두 Go2 위치·속도, QR 월드 정답, 접촉값을 제어 입력으로 넣지 않습니다.</p></div><div class="formula-card"><h3>수직 제어와 PX4의 역할</h3><p>네 방법 모두 <code>vx/vy/vz</code>를 제안합니다. QR가 보이지 않거나 중심·상대속도 gate를 만족하지 않으면 safety governor가 하강만 hold하고, 그 외에는 RL residual 또는 MPC가 수직 reference를 직접 정합니다. Offboard local-velocity setpoint를 수신한 PX4 multicopter 위치제어기가 자세·collective를 계산하고, 반환된 <code>HIL_ACTUATOR_CONTROLS</code> 네 값만 X500의 MuJoCo 추력/토크로 변환합니다. 직접 비행 wrench, 직접 pose/velocity 이동, 외부 모터 명령은 사용하지 않습니다.</p><p>최신 각 실행의 EKF innovation, HIL 메시지 수, trace CSV와 ULog는 아래 12개 카드에서 각각 확인할 수 있습니다.</p></div></div>
...
<p>각 단계 시작 보드에 <strong>PX4 SITL·EKF2·MAVLink HIL</strong>, QR/PnP 기반 Offboard <code>vx/vy/vz</code>, PX4 위치·자세 제어와 네 모터 할당을 명시했습니다. 이후에는 같은 실행의 3인칭과 하향 QR 카메라 동기화 화면을 재생합니다.</p><p class="links"><a href="{reel}">PX4 HIL 합본 MP4</a></p></section>'''
    ...
    return f'''<section class="section" id="px4-flat-hil-suite"><div class="section-kicker">3D RL + 3D CAMERA MPC → REAL PX4 EKF2 HIL · {expected} MP4</div><h2>PX4 평지 초급·중급·고급 — PPO·DDPG·SAC·MPC 3축 배포 검증</h2><div class="callout"><strong>{status}.</strong> PPO·DDPG·SAC는 동일 7D 카메라/자체수직속도 입력에서 <strong>3D 속도 residual [Δv<sub>x</sub>, Δv<sub>y</sub>, Δv<sub>z</sub>]</strong>을 학습한 정책입니다. <strong>MPC는 학습 모델이 아닌</strong> 카메라/PnP 상대위치·상대속도와 PX4 EKF 3축 속도로 8-step 3D 예측 비용을 최소화하는 기준선입니다. QR를 놓쳤거나 중심·상대속도가 안전 gate 밖이면 governor가 하강만 보류하며, 그 외 수평·수직 reference는 각 기법이 냅니다. 네 기법 모두 각 난이도에서 독립 PX4 SITL·EKF2·MAVLink HIL 프로세스로 실행했습니다. Go2는 별도의 학습된 12관절 저수준 PPO로 실제 발바닥 접지 보행하며, 영상 HUD와 표에서 실제 속도·이동거리·발바닥 접지 수를 확인할 수 있습니다. <strong>난이도 이동 설정:</strong> {route_summary}. 아래 MP4는 실제 PX4 모터 할당 경로의 배포 검증입니다.</div>{''.join(cards_by_algorithm)}{reel_section}<p class="links"><a href="px4_flat_hil_suite.json">전체 실행 manifest</a><a href="px4_flat_hil_training/px4_flat_hil_training_metrics.json">RL 재학습 평가 JSON</a><a href="px4_flat_hil_onnx_models.json">RL ONNX 검증 manifest</a></p></section>'''
...
def main() -> None:
    ...
    state_formula = equation(
        ...
        r"a_{res}^{xy}&=0.20\lambda(h)\max(0,\hat e^{\mathsf T}R_{WB,xy}a_{xy})\hat e",
        r"v_{xy}^{*} &= \operatorname{clip}_{3.6}\!\left(\hat v_{qr,xy}^{cam}+0.92(R_{WB}t_B^{qr})_{xy}+a_{res}^{xy}\right)\quad(\delta_{qr}=1)",
        r"v_z^{rel,*}&=v_{z,\mathrm{PnP\ nominal}}^{rel}+0.22\lambda(h)(R_{WB}[0,0,a_z]^\mathsf T)_z,\quad \operatorname{clip}_{[-0.85,0.45]}\ (\mathrm{visible\ approach})",
        r"h\le h_{final}&\Rightarrow \operatorname{clip}_{[-0.22,0.12]}\ \mathrm{and\ hold\ descent\ when\ visual\ gate\ fails}",
    ...
    reward_formula = equation(
        r"r_t &= 7(d_{t-1}-d_t)-0.035d_t+2\max(0,h_{t-1}-h_t)\mathbb{1}[\mathrm{aligned}]-0.06\lVert a_t\rVert_2^2+0.12\mathbb{1}[d_t<0.30]",
...
<div class="callout"><strong>정책은 하단 카메라 원본 영상을 직접 보지 않습니다.</strong> 카메라에서 QR 네 모서리를 찾아 만든 숫자 6개와 드론 자체 PX4 수직속도 1개를 정확한 순서의 <code>float32[7]</code> 배열로 묶어 0.1초마다 PPO·DDPG·SAC ONNX에 전달합니다. 이 PX4 HIL 3D 세트의 ONNX는 <code>[Δv<sub>x</sub>, Δv<sub>y</sub>, Δv<sub>z</sub>]</code>를 출력합니다. 착륙다리 접촉, Go2 위치·속도·경로, QR의 MuJoCo 정답 좌표는 이 배열에 들어가지 않습니다.</div>
...
<section class="section"><h2>PPO · DDPG · SAC 학습방법과 하이퍼파라미터</h2><div class="callout">세 기법은 위에서 설명한 <strong>동일한 float32[7] 입력, 3D residual 출력 [Δv<sub>x</sub>, Δv<sub>y</sub>, Δv<sub>z</sub>], 환경 보상</strong>을 사용합니다. 차이는 경험을 모으고 actor/critic을 갱신하는 방식입니다.</div><div class="example-grid"><article><h3>PPO</h3><p>현재 정책으로 512 step rollout을 모은 뒤, advantage가 좋은 행동 확률은 올리고 나쁜 행동 확률은 내립니다. 한 번의 큰 업데이트로 정책이 무너지는 것을 막기 위해 이전 정책과의 확률비를 ±20% 범위로 clip하며 같은 rollout을 10 epoch 재사용합니다.</p><p><code>lr 2.5e-4 · batch 128 · γ 0.997 · GAE 0.96 · clip 0.20 · value coefficient 0.50</code></p></article><article><h3>DDPG</h3><p>결정론 actor가 한 행동을 critic이 평가합니다. 180,000개 replay buffer에서 과거 transition을 무작위로 뽑아 Q 오차를 줄이고, actor는 critic의 Q가 커지는 방향으로 갱신합니다. 처음 2,000 step은 buffer를 채우며, 학습 탐색에는 표준편차 0.18 행동 잡음을 사용합니다.</p><p><code>lr 3e-4 · batch 256 · γ 0.997 · target τ 0.01 · warm-up 2,000</code></p></article><article><h3>SAC</h3><p>두 Q critic 중 작은 값을 사용해 과대평가를 줄입니다. actor는 높은 Q뿐 아니라 행동 분포의 entropy도 유지하도록 학습해, DDPG보다 확률적인 탐색을 합니다. entropy 계수는 초기값 0.02에서 자동 조정됩니다.</p><p><code>lr 3e-4 · replay 180,000 · batch 256 · γ 0.997 · target τ 0.01 · warm-up 2,000</code></p></article><article><h3>공통 데이터 한 건</h3><p>각 transition은 <code>(현재 7D, 3D 행동, reward, 다음 7D, 종료 여부)</code>입니다. 보상과 종료 판정에는 MuJoCo 정답 라벨을 쓸 수 있지만, 저장된 actor가 받는 입력은 계속 7D뿐입니다. 평가와 영상에서는 학습 탐색 잡음을 끄고 결정론적으로 ONNX 출력을 사용합니다.</p></article></div><details><summary>PPO·DDPG·SAC 손실 수식 펼쳐 보기</summary>{loss_formula}</details><p>수식은 CDN이 아닌 이 서버의 로컬 MathJax로 렌더링됩니다.</p></section>
{flat_reels_html}
...
<footer>6개 핵심 평가지표: <code>mean_reward</code>, <code>std_reward</code>, <code>success_rate</code>, <code>mean_terminal_error_m</code>, <code>mean_episode_duration_s</code>, <code>mean_episode_steps</code>. 접촉·데크 속도·Go2 물리량은 정책 입력이 아닌 <code>offline_sim_*</code> 진단입니다. 모든 산출물은 MuJoCo에서 생성했습니다.</footer></main></body></html>'''
    # The archival Go2-only report text below shares the same generated page.
    # Replace its old two-axis wording so it cannot be mistaken for the new
    # PX4 HIL 3D policy contract rendered above.
    document = document.replace(
        "모델 출력도 모터 PWM·추력·최종 속도 명령이 아니라, 기본 시각서보에 아주 작게 더할 수 있는 수평 보정 제안 두 개입니다.",
        "모델 출력은 모터 PWM·추력·최종 속도 명령이 아니라, 카메라/PnP 기준 속도에 제한적으로 더하는 3축 residual 제안 세 개입니다.",
    ).replace(
        "ONNX 출력 2개는 실제로 무엇을 하나",
        "ONNX 출력 3개는 실제로 무엇을 하나",
    ).replace(
        "<code>a_x, a_y ∈ [-1,1]</code>은 드론 body XY 방향의 무차원 residual 제안입니다.",
        "<code>a_x, a_y, a_z ∈ [-1,1]</code>은 드론 body XYZ 방향의 무차원 residual 제안입니다.",
    ).replace(
        "제어기는 이를 현재 드론 자세로 world XY에 회전한 다음, 카메라가 측정한 QR 중심을 향하는 안쪽 성분만 남깁니다. QR에서 멀어지는 성분과 접선 성분은 버립니다.",
        "제어기는 XY를 현재 드론 자세로 world XY에 회전한 뒤 QR 중심을 향하는 안쪽 성분만 남기고, Z는 카메라/PnP 명목 하강에 더합니다. QR에서 멀어지는 XY 성분과 시각 gate 밖의 하강은 버립니다.",
    ).replace(
        "held-out 평가·ONNX 영상에서 남은 값의 최대 영향은 <strong>0.001 m/s</strong>입니다.",
        "PX4 HIL 3D 평가·ONNX 영상에서 XY residual 최대 영향은 <strong>0.20 m/s</strong>, Z residual 최대 영향은 <strong>0.22 m/s</strong>입니다.",
    ).replace(
        "상대높이 1.20 m 아래부터 선형으로 줄고 0.45 m 이내에서는 완전히 0이 됩니다.",
        "상대높이 1.20 m 아래부터 residual envelope을 줄이며, 최종 정밀구간에서는 Z를 -0.22~+0.12 m/s로 제한합니다.",
    ).replace(
        "실제 빠른 이동, 목표속도 feed-forward, 중심 복원, 단계별 하강과 자세 정렬은 정책 밖 결정론적 카메라/IMU 제어기가 담당하며 수평 목표속도는 3.6 m/s로 제한됩니다. 즉 RL 모델 하나가 착륙기 전체를 직접 조종하는 구조가 아닙니다.",
        "빠른 이동과 PnP feed-forward는 기준 속도를 만들고, RL은 세 축을 보정합니다. 시각 gate·충격 복구·최종 속도 한계는 별도 안전층이며, 수평 목표속도는 3.6 m/s로 제한됩니다. PX4가 그 velocity setpoint로 자세·collective·모터를 계산합니다.",
    ).replace(
        "동일한 float32[7] 입력, 2D residual 출력, 환경 보상",
        "동일한 float32[7] 입력, 3D residual 출력 [Δvx, Δvy, Δvz], 환경 보상",
    ).replace(
        "(현재 7D, 2D 행동, reward, 다음 7D, 종료 여부)",
        "(현재 7D, 3D 행동, reward, 다음 7D, 종료 여부)",
    )
    document = document.replace(
        "8-step 3D 속도 계획을 매 100 ms 다시 풉니다.",
        "8-step 3D 속도 계획을 매 100 ms 다시 풉니다. 수평 위치/상대속도/종단 위치 가중치는 28.0/1.5/80.0이고, 수직 위치/상대속도/종단 위치는 10.0/8.0/24.0입니다. Z 목표는 QR 중심이 아닌 stock X500 스키드 접지 clearance이며, 그보다 높을 때 최소 하강 후보 0.22 m/s를 MPC 제약으로 둡니다.",
    )
