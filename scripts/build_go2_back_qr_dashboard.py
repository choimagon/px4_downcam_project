...
<div class="split"><div class="formula-card"><h3>지형은 무엇으로 만들었나</h3><p>공개 경사는 길이 16 m의 <strong>10% grade (5.71°) 회전 box collision surface</strong>입니다. 요철은 폭 2.4 m의 <strong>연속 MuJoCo heightfield 충돌면</strong>이며, 수직 타일 벽이 아닌 걸을 수 있는 연속 기복으로 모델링했습니다. 높이 진폭은 <strong>24 / 48 / 80 mm</strong>입니다. 저대비 흙·잔디 입자 재질은 같은 충돌면의 시각 재질일 뿐이고, 별도 돌·카메라 전용 바닥·발 통과 오브젝트는 없습니다.</p><p>상승·하강 모두 12 m 코스를 통과하기 전, 유한 collision surface 끝으로 걸어 내려가지 않도록 감속·정지합니다.</p></div><div class="formula-card"><h3>무엇을 검증했나</h3><p>Go2 저수준 PPO 후보의 상태 계약은 IMU·관절·속도명령·직전 행동 450D입니다. 다만 이 지형 공개 세트는 후보가 낙상 기준을 통과하지 못했으므로, 실제 IMU·자체 odometry 되먹임의 물리 기준 트로트를 사용합니다. 지형 높이·QR pose·드론 상태는 Go2 제어 입력에 넣지 않고, Go2 root wrench는 항상 0입니다.</p><p>아래 영상은 결합 Go2+X500 물리 장면에서 고정 시드를 사전 재생해 <strong>Go2 낙상 0, 최고 기울기 40° 이하, 두 스키드 물리 접촉, root wrench 0</strong>을 통과한 재현입니다. 이는 넓은 무작위화 강건성 인증이 아니라 공개된 고정 시드 재생 검증입니다.</p></div></div>
